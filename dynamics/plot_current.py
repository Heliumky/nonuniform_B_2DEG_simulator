"""Public stage 3: plot a saved HDF5 current dataset without recalculation."""
...
def _default_output_path(current_path):
    """Put a user-selected current movie in the normal figures directory."""
    return config.OUTPUT_DIRECTORY / f"{Path(current_path).stem}.gif"
...
def main(current_path=None, output_path=None, frame_stride=None):
    """Load one chosen current HDF5 file only, then save its GIF."""
    if current_path is None:
        x, times, jx, jy = load_configured_current()
        _, _, _, _, metadata = load_current_file(config.CURRENT_DATA_FILE)
        current_path = config.CURRENT_DATA_FILE
    else:
        current_path = _resolve_current_path(current_path)
        x, times, jx, jy, metadata = load_current_file(current_path)
    output_path = (config.CURRENT_GIF if current_path == config.CURRENT_DATA_FILE and output_path is None
    ...
    figure.suptitle(
        rf"Driven channel currents  |  $k_y={trajectory_metadata['ky'] / config.AU_TO_NM:.2f}\,\mathrm{{nm}}^{{-1}}$"
...
def _validate_comparable(first, second):
    ...
    if not np.array_equal(x_a, x_b) or not np.array_equal(times_a, times_b):
        raise ValueError("Current files have different x or time grids and cannot be compared directly")
    ...
    if remaining_a != remaining_b:
        raise ValueError("Current files use different reconstruction-grid metadata")
    return waveform_a, waveform_b, trajectory_a


def plot_comparison(first_path, second_path, output_path=None):
    """Plot integrated and local current differences for two saved HDF5 files."""
    first_path = _resolve_current_path(first_path)
    second_path = _resolve_current_path(second_path)
    first = load_current_file(first_path)
    second = load_current_file(second_path)
    ...
    integrated = (
        np.trapezoid(jx_a, x, axis=1), np.trapezoid(jx_b, x, axis=1),
        np.trapezoid(jy_a, x, axis=1), np.trapezoid(jy_b, x, axis=1),
    )

    figure, axes = plt.subplots(2, 2, figsize=(12, 8), layout="constrained")
    labels = (waveform_a, waveform_b)
    colors_by_waveform = ("#c43c39", "#007c78")
    for axis, values_a, values_b, title, ylabel in (
        (axes[0, 0], integrated[0], integrated[1], r"Integrated longitudinal current", r"$\int J_x\,dx$ (a.u.)"),
        (axes[0, 1], integrated[2], integrated[3], r"Integrated transverse current", r"$\int J_y\,dx$ (a.u.)"),
    ):
        axis.plot(time_periods, values_a, color=colors_by_waveform[0], lw=1.5, label=labels[0])
        axis.plot(time_periods, values_b, color=colors_by_waveform[1], lw=1.5, label=labels[1])
        axis.set_title(title)
        axis.set_xlabel(r"$t/T_{\rm ac}$")
        axis.set_ylabel(ylabel)
        axis.grid(color="0.88", lw=0.8)
        axis.legend(title="waveform")

    for axis, values, title, label in (
        (axes[1, 0], delta_jx, rf"$\Delta J_x=J_x^{{{waveform_a}}}-J_x^{{{waveform_b}}}$", r"$\Delta J_x$ (a.u.)"),
        (axes[1, 1], delta_jy, rf"$\Delta J_y=J_y^{{{waveform_a}}}-J_y^{{{waveform_b}}}$", r"$\Delta J_y$ (a.u.)"),
    ):
...
def plot_overlay(first_path, second_path, output_path=None, frame_stride=None):
    """Animate spatial currents from two comparable HDF5 files on shared axes."""
    first_path = _resolve_current_path(first_path)
    second_path = _resolve_current_path(second_path)
    first = load_current_file(first_path)
    second = load_current_file(second_path)
    ...
    data = ((jx_axis, jx_a, jx_b, r"$L_yj_x$ (a.u.)", r"Longitudinal current $J_x$"),
            (jy_axis, jy_a, jy_b, r"$L_yj_y$ (a.u.)", r"Transverse current $J_y$"))
    lines = []
    ...
    time_text = jx_axis.text(0.98, 0.88, "", transform=jx_axis.transAxes,
                               ...
                               bbox={"boxstyle": "round,pad=0.25", "fc": "white", "ec": "0.75"})
    figure.suptitle(
        rf"Spatial-current overlay: {waveform_a} (solid) vs {waveform_b} (dashed)"
        + "\n"
        + rf"$k_y={metadata['ky'] / config.AU_TO_NM:.2f}\,\mathrm{{nm}}^{{-1}}$, "
    ...
    base = Path(first_path).stem.replace(f"current_y6_3_{waveform_a}_", "")
    output = (config.OUTPUT_DIRECTORY / f"current_overlay_{waveform_a}_vs_{waveform_b}_{base}.gif"
              if output_path is None else Path(output_path))
    output.parent.mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Plot one current GIF or compare two current HDF5 files.")
    ...
    comparison_mode.add_argument("--compare", nargs=2, metavar=("FIRST_H5", "SECOND_H5"),
                                 help="write integrated-current overlays and local-current differences as PNG")
    comparison_mode.add_argument("--overlay", nargs=2, metavar=("FIRST_H5", "SECOND_H5"),
                                 help="write a spatial-current overlay GIF")
    parser.add_argument("current", nargs="?", help="current HDF5 to animate (default: configured current data)")
    ...
    try:
        if arguments.compare:
            ...
            main(arguments.current, arguments.output, arguments.frame_stride)
